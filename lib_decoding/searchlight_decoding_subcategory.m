...
function ret = searchlight_decoding_subcategory(subj_single_dir, mask_output_dir, decoding_result_dir, radius)
...
subj_single_output_dir = fullfile(decoding_result_dir, ['searchlight_radius_', num2str(radius), '_subcategory']);
...
label_names = { 'regexp:.*GreatestCommonDivider.*', ...
                'regexp:.*Power.*', ...
                'regexp:.*PrimeNumber.*', ...
                'regexp:.*BinarySearch.*', ...
                'regexp:.*LinerSearch.*', ...
                'regexp:.*BubbleSort.*', ...
                'regexp:.*InsertionSort.*', ...
                'regexp:.*SelectionSort.*', ...
                'regexp:.*FindingAWord.*', ...
                'regexp:.*FindingMinimumString.*', ...
                'regexp:.*RunLength.*' ...
               };
...
%% set cost weight for each class
tab = tabulate(cfg.design.label(:,1));
wi  = 0.5 ./ (tab(:,2)/max(tab(:,2))); % weights for class(i)
disp('Using correct (balanced) weights');
weight_str = [''];
for ci = 1:length(wi)
    wstr = ['-w' num2str(ci) ' ' num2str(wi(ci))];
    weight_str = [weight_str ' ' wstr];
end%ind
...
train_model_parameters  = ['-s 0 -t 0 -c 0.1 -m 1000' weight_str ' -q'];
cfg.decoding.train.classification.model_parameters  = train_model_parameters;
